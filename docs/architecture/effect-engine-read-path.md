...
| `temp_hp` / `heal` / `spend_resource` / `recover_resource` / `recover_resource_to_max` / `toggle_combat_flag` / `sync_companion` / `companion_command` / `table_roll` / `feature_dc` / `heal_from_dice_pool` / `grant_inspiration` / `check_boost` / `catalog_maneuver` / `strike_self_cost` / `apply_condition` / `clear_condition` | numeric/note/dice/flag/companion/condition | `executeCatalogEffect` + `applyDeclaredEconomyTableAction` / `applyCastSheetEffects` | session spend / table-action / **cast** |
