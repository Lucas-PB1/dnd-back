...
ADR: [`adr-effect-engine.md`](adr-effect-engine.md) · Dicionário: [`effect-dictionary.md`](effect-dictionary.md) · Residual mesa: [`../plans/effect-mesa-checklist.md`](../plans/effect-mesa-checklist.md) · Magia skirmish/duelo: [`spell-combat.md`](spell-combat.md) (`phb_spell_combat` + `resolveCombatSpell` — fora deste motor)
...
| `temp_hp` / `heal` / `spend_resource` / `recover_resource` / `recover_resource_to_max` / `toggle_combat_flag` (`rage`/`reckless`/`sacred_weapon`) / `sync_companion` / `companion_command` / `table_roll` (+ `phb_effect_table_roll`) / `feature_dc` / `heal_from_dice_pool` / `grant_inspiration` / `check_boost` / `catalog_maneuver` / `strike_self_cost` / `apply_condition` / `clear_condition` | numeric/note/dice/flag/companion/condition/table_roll/temp_hp | `executeCatalogEffect` + `applyDeclaredEconomyTableAction` / `applyCastSheetEffects` | session spend / table-action / **cast** |
| `slot_refund_on_die_match` | dice + numeric (máx. círculo) | `applySlotRefundOnCast` | `consumeNonItemCastCost` |
