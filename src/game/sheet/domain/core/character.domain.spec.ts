...
import { abilityModifier } from '../stats/ability-modifier';

const DWARF_TENACITY: HitPointsBonusRow = { label: 'Tenacidade Anã', perLevel: 1 };
const DRACONIC_RESILIENCE: HitPointsBonusRow = {
  label: 'Resiliência Dracônica',
  perLevel: 1,
  fromLevel: 3,
};
const TOUGH: HitPointsBonusRow = { label: 'Vigoroso', perLevel: 2 };
const BOON_OF_FORTITUDE: HitPointsBonusRow = {
  label: 'Dádiva da Fortitude',
  flat: 40,
};
...
describe('Character domain (PHB)', () => {
  ...
  describe('hitPointsBonus', () => {
    it('is zero without permanent sources', () => {
      expect(hitPointsBonus(5, [])).toBe(0);
    });

    it('dwarf gains +1 per character level', () => {
      expect(hitPointsBonus(1, [DWARF_TENACITY])).toBe(1);
      expect(hitPointsBonus(7, [DWARF_TENACITY])).toBe(7);
    });

    it('draconic resilience only applies from sorcerer level 3', () => {
      expect(hitPointsBonus(2, [DRACONIC_RESILIENCE])).toBe(0);
      expect(hitPointsBonus(3, [DRACONIC_RESILIENCE])).toBe(3);
      expect(hitPointsBonus(10, [DRACONIC_RESILIENCE])).toBe(10);
    });

    it('tough feat gains +2 per character level', () => {
      expect(hitPointsBonus(4, [TOUGH])).toBe(8);
    });

    it('boon of fortitude adds a flat +40', () => {
      expect(hitPointsBonus(19, [BOON_OF_FORTITUDE])).toBe(40);
    });
  ...
  describe('calculateHitPointsMax with permanent bonuses', () => {
    ...
    it('adds the dwarf tenacity to the class base', () => {
      expect(
        calculateHitPointsMax(5, fighterProfile, 14, [DWARF_TENACITY]),
      ).toBe(44 + 5);
    });

    it('adds the tough feat at level 1', () => {
      expect(
        calculateHitPointsMax(1, fighterProfile, 10, [TOUGH]),
      ).toBe(12);
    });
  });
