...
/** Tipos elegíveis de Matar Monstro (Explorador de Masmorras). */
export const DUNGEONEER_SLAYER_TYPES = [
  'Aberração',
  'Dragão',
  'Feérico',
  'Corruptor',
...
export function fighterCombatNotes(input: {
  ...
  if (input.subclassSlug === 'dungeoneer') {
    ...
    if (level >= 10) {
      notes.push(
        `Matar Monstro: +1d10 1×/turno vs ${DUNGEONEER_SLAYER_TYPES.join(', ')}`,
      );
    }
    if (level >= 15) {
      notes.push(
