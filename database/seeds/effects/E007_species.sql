...
USING rpg.phb_species sp
WHERE e.owner_kind = 'species'
  AND e.owner_id = sp.id
  AND (
    e.trigger NOT IN ('on_resource_spend', 'on_table_action')
    OR e.resource_slug IN (
      'stonecunning', 'dragonFlight', 'celestialRevelation', 'relentlessEndurance'
    )
  );
