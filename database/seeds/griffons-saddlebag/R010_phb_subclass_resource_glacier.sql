...
SELECT
  ...
  rd.id,
  3,
  'constitution_mod'::rpg.resource_max_formula,
  NULL,
  sf.id,
  FALSE,
  FALSE,
...
JOIN rpg.phb_resource_definition rd ON rd.slug = 'glacier-rage-extension'
LEFT JOIN rpg.phb_subclass_feature sf
  ON sf.subclass_id = s.id AND sf.name = 'Permafrost'
WHERE s.slug = 'path-of-the-glacier'
