...
-- Lote A: hit_die is now enum literal

INSERT INTO rpg.phb_class (slug, name, tagline, summary, description, primary_ability_label, primary_ability_operator, hit_die, hp_level1_die_value, hp_fixed_per_level, hp_minimum_gain_per_level, hp_constitution_mod_applies, subclass_unlock_level, subclass_label, skill_choice_count, skill_choice_from, source_citation_id, spell_slot_pattern_id)
VALUES
...
Bárbaros perfeitamente adequados para aventuras.', 'Força', NULL, 'd12', 12, 7, 1, TRUE, 3, 'Trilha', 2, NULL, (SELECT id FROM rpg.phb_source_citation WHERE slug = 'phb-2024-pt:ch3:55-182'), NULL),
...
da magia distinguem os Bardos dos demais.', 'Carisma', NULL, 'd8', 8, 5, 1, TRUE, 3, 'Colégio', 3, 'any', (SELECT id FROM rpg.phb_source_citation WHERE slug = 'phb-2024-pt:ch3:55-182'), (SELECT id FROM rpg.phb_spell_slot_pattern WHERE slug = 'full')),
  ('warlock', 'Bruxo', 'Ocultista fortalecido com pactos sobrenaturais', 'Conjure magias derivadas de conhecimento oculto.', NULL, 'Carisma', NULL, 'd8', 8, 5, 1, TRUE, 3, 'Patrono', 2, NULL, (SELECT id FROM rpg.phb_source_citation WHERE slug = 'phb-2024-pt:ch3:55-182'), (SELECT id FROM rpg.phb_spell_slot_pattern WHERE slug = 'pact')),
  ('cleric', 'Clérigo', 'Campeão divino da magia sagrada', 'Invoque magia divina para curar, fortalecer e castigar.', NULL, 'Sabedoria', NULL, 'd8', 8, 5, 1, TRUE, 3, 'Domínio', 2, NULL, (SELECT id FROM rpg.phb_source_citation WHERE slug = 'phb-2024-pt:ch3:55-182'), (SELECT id FROM rpg.phb_spell_slot_pattern WHERE slug = 'full')),
  ('druid', 'Druida', 'Guardião da natureza e da magia primal', 'Canalize a magia da natureza para curar, moldar e controlar os elementos.', NULL, 'Sabedoria', NULL, 'd8', 8, 5, 1, TRUE, 3, 'Círculo', 2, NULL, (SELECT id FROM rpg.phb_source_citation WHERE slug = 'phb-2024-pt:ch3:55-182'), (SELECT id FROM rpg.phb_spell_slot_pattern WHERE slug = 'full')),
...
poderiam conceder um poder maior do que o disponível na atualidade.', 'Inteligência', NULL, 'd6', 6, 4, 1, TRUE, 3, 'Tradição', 2, NULL, (SELECT id FROM rpg.phb_source_citation WHERE slug = 'phb-2024-pt:ch3:55-182'), (SELECT id FROM rpg.phb_spell_slot_pattern WHERE slug = 'full')),
  ('monk', 'Monge', 'Artista marcial de velocidade e precisão', 'Entre e saia do combate corpo a corpo com velocidade e precisão.', NULL, 'Destreza e Sabedoria', 'and', 'd8', 8, 5, 1, TRUE, 3, 'Tradição', 2, NULL, (SELECT id FROM rpg.phb_source_citation WHERE slug = 'phb-2024-pt:ch3:55-182'), NULL),
...
ON CONFLICT (slug) DO UPDATE SET
  ...
  primary_ability_operator = EXCLUDED.primary_ability_operator,
  hit_die = EXCLUDED.hit_die,
  hp_level1_die_value = EXCLUDED.hp_level1_die_value,
  hp_fixed_per_level = EXCLUDED.hp_fixed_per_level,
  hp_minimum_gain_per_level = EXCLUDED.hp_minimum_gain_per_level,
  hp_constitution_mod_applies = EXCLUDED.hp_constitution_mod_applies,
