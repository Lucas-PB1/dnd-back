...
-- Garante o pattern `third` (também reutilizado por Spellslinger / Trapaceiro Arcano).
...
WHERE p.slug = 'third'
ON CONFLICT (pattern_id, level, circle) DO UPDATE SET
...
ON CONFLICT (subclass_id, level) DO UPDATE SET
