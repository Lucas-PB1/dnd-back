-- Cap.6 choice rules (stage mode / auto / pick / sub / requireMatch).

DO $$ BEGIN
  CREATE TYPE rpg.transformation_stage_mode AS ENUM (
    'auto_all',
    'pick1',
    'pick2',
    'fixed_plus_pick1',
...
CREATE TABLE IF NOT EXISTS rpg.phb_transformation_stage_rule (
...
CREATE INDEX IF NOT EXISTS idx_phb_transformation_stage_rule_feat
  ON rpg.phb_transformation_stage_rule (feat_id);

CREATE TABLE IF NOT EXISTS rpg.phb_transformation_stage_auto_boon (
...
CREATE TABLE IF NOT EXISTS rpg.phb_transformation_stage_pick_key (
  stage_rule_id BIGINT NOT NULL
    REFERENCES rpg.phb_transformation_stage_rule(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS rpg.phb_transformation_sub_option (
  id BIGSERIAL PRIMARY KEY,
  feat_id BIGINT NOT NULL REFERENCES rpg.phb_feat(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_phb_transformation_sub_option_feat
  ON rpg.phb_transformation_sub_option (feat_id);

CREATE TABLE IF NOT EXISTS rpg.phb_transformation_require_match (
  id BIGSERIAL PRIMARY KEY,
  feat_id BIGINT NOT NULL REFERENCES rpg.phb_feat(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS rpg.phb_transformation_require_match_pair (
