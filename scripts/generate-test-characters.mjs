...
const PRESERVE_FILES = new Set(["marcus.json", "aelindra.json"]);
...
function cleanGeneratedFiles() {
  for (const file of fs.readdirSync(charsDir)) {
    if (!file.endsWith(".json") || PRESERVE_FILES.has(file)) continue;
...
for (const bp of blueprints) {
  try {
    ...
    console.error(`✗ ${bp.id}: ${err.message}`);
    fail++;
  }
}

const preserved = [...PRESERVE_FILES].filter((f) => fs.existsSync(path.join(charsDir, f))).length;
console.log(
  `\n${ok} ficha(s) gerada(s), ${fail} falha(s), ${preserved} preservada(s) — total ${ok + preserved} em data/characters/`
);
